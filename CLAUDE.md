...
- The project folder structure is as follows:
  ...
  - skills: Folder containing Claude skills for compound-context methodology
    ...
    - util-updprd: Applies a free-form requirement change to the PRD.md files across the project. Analyzes every application's PRD.md to determine impacted applications, modules and subsections, decides per item whether to add/cancel/modify User Stories, NFRs, Constraints, References and Tests, auto-classifies the change as major/minor/patch, and layers the new/updated items under a NEW version tag on top of the existing versions in each PRD.md (prior versions immutable). Tags new items via util-ustagger and appends a CHANGELOG.md entry per impacted application. (Prerequisite: PRD.md, CLAUDE.md)
    ...
    - specgen-react-tailwind: Generates React 19 SPA technical specification styled with Tailwind CSS v3 (utility-first) and Headless UI v2 + Heroicons for accessible primitives — instead of a component framework. Keeps the same data stack: TypeScript 5, Vite 6, React Router v7, TanStack Query v5, Zustand v5, React Hook Form v7, and Zod v3. Optional components map to the Tailwind ecosystem: TanStack Table (grids), Recharts (charts), react-day-picker (dates), Tiptap (rich text). (Prerequisite: PRD.md, modelgen-*, mockgen-tailwind or mockgen-shadcn)
    - specgen-nextjs-react-tailwind: Generates Next.js 15 full-stack web application technical specification with the App Router — React 19 Server Components by default, Server Actions (Zod-validated) for all mutations, Drizzle ORM + SQLite (better-sqlite3) with committed drizzle-kit migrations, tiered authentication (auto-selects the lightest tier that satisfies the PRD: none / OIDC via openid-client / hand-rolled scrypt sessions / Better Auth — all behind a single src/lib/auth.ts entry point), Tailwind CSS v4 (CSS-first config) + shadcn/ui (vendored), Biome (single lint+format tool), Vitest (integration tests against in-memory SQLite with real migrations), Playwright E2E, Pino logging, and pnpm. No separate backend API, no client-side data-fetching library, no global state manager. Optional components stay within the shadcn ecosystem: Data Table (TanStack Table), Charts (Recharts), Calendar (react-day-picker); Route Handlers generated only for external consumers. (Prerequisite: PRD.md, modelgen-relational, mockgen-tailwind or mockgen-shadcn)
    - specgen-flutter-riverpod: Generates Flutter 3 mobile application technical specification with Dart 3, Riverpod 2 (with riverpod_generator), Hive 2 local storage, Dio 5 + dio_smart_retry, go_router 14, freezed + json_serializable, Firebase Messaging + flutter_local_notifications, cached_network_image, flutter_svg, pull_to_refresh, font_awesome_flutter, material_design_icons_flutter, intl, url_launcher, and flutter_native_splash. (Prerequisite: PRD.md, modelgen-*, mockgen-tailwind)
    ...
    - specgen-ts-cli: Generates TypeScript CLI application technical specification with Commander.js, tsup bundling, and optional @yao-pkg/pkg binary packaging. (Prerequisite: PRD.md, modelgen-*)
    - specgen-react-mui: Generates React SPA technical specification with React 19, Vite 6, MUI v6, TanStack Query, Zustand, and optional Keycloak authentication. (Prerequisite: PRD.md, modelgen-*, mockgen-tailwind)
    ...
    - tracegen-matrix: Generates/updates a per-application `context/TRACEABILITY.md` linking every PRD requirement ID (User Stories, NFRs, Constraints) and bug code to the source code that implements it. Resolves links from the in-source CO2 traceability comments (`Implements:`/`NFR:`/`Constraints:`/`Bug fixes:`) written by conductor-feature-develop and conductor-defect, with a name-based source-scan fallback, and OPTIONAL precision enrichment from the codebase-memory MCP when installed (works fully without it). Invoked automatically as the final traceability step of conductor-feature-develop and conductor-defect; also runnable manually. (Prerequisite: PRD.md, specification/*/SPEC.md, application source code)
