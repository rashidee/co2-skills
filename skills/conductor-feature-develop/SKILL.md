...
4. If it exists, read it and determine the current state:
   ...
   - If ALL modules are COMPLETED:
     ...
     - If no version argument, or the version is already tracked → output
...
3. **Check if ALL modules are now COMPLETED**:
   ...
   - If ALL modules have Status = COMPLETED:
     - Update IMPLEMENTATION_MASTER.md: set top-level `**Status**:` to `COMPLETED`
     - Output the Ralph Loop completion promise: `<promise>ALL MODULES IMPLEMENTED</promise>`
