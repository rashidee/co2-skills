...
`DEPLOYMENT.md`) by invoking the `depgen-ansible-k8s` skill. This phase runs
**before** README generation so the README can reference the deployment files.

#### Step 5.1: Invoke depgen-ansible-k8s

Invoke the deployment artifact generator:
```
Skill(skill: "depgen-ansible-k8s", args: "<application>")
```

The `depgen-ansible-k8s` skill auto-detects the technology stack from the application's
...
- `Dockerfile` — production-ready, multi-stage Docker build optimized for the application's
  technology stack
- `DEPLOYMENT.md` — comprehensive deployment specification with Kubernetes manifests and
  Ansible playbook specifications
...
- `<source-code-path>/DEPLOYMENT.md` exists
...
<If Dockerfile and DEPLOYMENT.md were generated by Phase 5, include this section.
...
See [DEPLOYMENT.md](DEPLOYMENT.md) for the complete deployment specification including
Kubernetes manifests (Deployment, Service, ConfigMap, Secret, Ingress) and Ansible
playbook specifications for automated deployment.
```
...
17. **Deployment artifacts and README.md are mandatory before completion** — After ALL modules
    ...
    `depgen-*` skill to generate `Dockerfile` and `DEPLOYMENT.md`. Phase 6 generates the
    ...
    linking to `DEPLOYMENT.md` if deployment artifacts were generated.
