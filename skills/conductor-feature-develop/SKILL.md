...
After ALL modules are COMPLETED, generate deployment artifacts (`Dockerfile` and
`DEPLOYMENT.yaml`) by invoking the appropriate `depgen-*` skill. This phase runs
**before** README generation so the README can reference the deployment files.

#### Step 5.1: Determine Technology Stack

Read `<app_folder>/context/specification/SPECIFICATION.md` to determine the application's
technology stack. This determines which `depgen-*` skill to invoke:

| Technology Stack | depgen Skill |
|---|---|
| Spring Boot (JTE, REST API) | `depgen-spring` |
| Laravel (Blade) | `depgen-laravel` |
| React (MUI, Vite) | `depgen-react` |
| Node.js CLI (TypeScript) | `depgen-ts-cli` |

#### Step 5.2: Invoke depgen Skill

Invoke the appropriate `depgen-*` skill:
```
Skill(skill: "depgen-<stack>", args: "<application> version:<version>")
```

The `depgen-*` skill will generate:
- `Dockerfile` — multi-stage Docker build optimized for the application's technology stack
- `DEPLOYMENT.yaml` — Kubernetes deployment manifest with service, configmap, and
  environment variable references

Both files are generated in the application's source code root (`<source-code-path>/`).

#### Step 5.3: Verify Deployment Artifacts

Confirm that both files were generated:
- `<source-code-path>/Dockerfile` exists
- `<source-code-path>/DEPLOYMENT.yaml` exists
...
Initializr) and does not reflect the actual implemented application. Phase 6 always produces
...
#### Step 6.1: Read SPECIFICATION.md
...
<If Dockerfile and DEPLOYMENT.yaml were generated by Phase 5, include this section.
...
```bash
docker build -t <artifact-id>:<version> .
```
...
See [DEPLOYMENT.yaml](DEPLOYMENT.yaml) for the full Kubernetes deployment manifest
including Service, ConfigMap, and environment variable configuration.
```
...
#### Step 6.3: Update Tracking and Complete
...
17. **Deployment artifacts and README.md are mandatory before completion** — After ALL modules
    are COMPLETED, Phase 5 (Generate Deployment Artifacts) and Phase 6 (Generate README.md)
    MUST both run before outputting the completion promise. Phase 5 invokes the appropriate
    `depgen-*` skill to generate `Dockerfile` and `DEPLOYMENT.yaml`. Phase 6 generates the
    README with content derived from SPECIFICATION.md — do NOT invent stack details, run
    commands, or architecture descriptions. The README must include a Deployment section
    linking to `DEPLOYMENT.yaml` if deployment artifacts were generated.
