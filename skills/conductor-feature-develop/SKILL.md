...
Kubernetes manifests) by invoking the `depgen-k8s` skill. This phase runs
**before** README generation so the README can reference the deployment files.

#### Step 5.1: Invoke depgen-k8s
...
The `depgen-k8s` skill auto-detects the technology stack from the application's
...
<If Dockerfile and Kubernetes manifests were generated by Phase 5, include this section.
